...
export default function AdminLayout({ user, onLogout, children }) {
  ...
  useEffect(() => {
    ...
    if (mainEl) {
      mainEl.classList.add('admin-portal-full-bleed');
    }
    return () => {
      if (mainEl) {
        mainEl.classList.remove('admin-portal-full-bleed');
      }
    };
  }, []);
  ...
  return (
    <div style={{
      ...
      <main className="portal-main-content" style={{
        ...
        <div className="admin-mobile-toggle" style={{ display: 'none', marginBottom: '1rem' }}>
          <button
          ...
          >
            <Menu size={18} /> Admin Navigation Menu
          </button>
        </div>
