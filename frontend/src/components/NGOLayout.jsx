...
export default function NGOLayout({ user, onLogout, children }) {
  ...
  useEffect(() => {
    ...
    if (mainEl) {
      mainEl.classList.add('ngo-portal-full-bleed');
    }
    return () => {
      if (mainEl) {
        mainEl.classList.remove('ngo-portal-full-bleed');
      }
    };
  }, []);
  ...
  return (
    <div style={{
      ...
      {/* Desktop Sidebar */}
      <div className="ngo-sidebar-desktop" style={{ display: 'flex' }}>
        <NGOSidebar user={user} onLogout={onLogout} />
      </div>
      ...
      <main className="portal-main-content" style={{
        ...
        <div className="ngo-mobile-toggle" style={{ display: 'none', marginBottom: '1rem' }}>
          <button
          ...
          >
            <Menu size={18} /> NGO Navigation Menu
          </button>
        </div>
